...
-- "_ORACLE_SCRIPT" --> 옛날 오라클 방식
--> 계정명이 언급되는 상황에서는 위에 구문 반드시 실행!!
   --> 이유: 원래 계정명 앞에 클라우드 연결위한 구문(C##) 써야하나 안쓰기 위해

-- (KH 계정 접속)
-- VIEW 생성 권한 부여
GRANT CREATE VIEW TO KH_HYJ;
...
--> 만들어지 DEPT_COPY2 테이블을 확인해보면
-- DEPT_TLTLE 컬럼만 NULL이 허용되어 있음!
...
-- VIEW에 INSERT를 수행했지만
-- VIEW를 만들 때 사용한 원본 테이블에 값이 INSERT 됨을 확인!!
