 ...
 * 	 조회 결과(RESULT SET)에서 모든 행에 
 * 	 산술 연산이 적용된 결과 값이 조회된다!!
 */
 ...
 *   DISTINCT가 지정된 컬럼의 중 중복되는 값이 존재할 경우
...
-- EMPLOYEE 테이블에서
-- 부서 코드가 'D6'인 사원 중
-- 급여가 400만을 초과하는 사원의 이름, 부서 코드, 급여 조회
...
-- EMPLOYEE 테이블에서
-- 급여가 300만 이상 500만 미만인 사원의 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY 
FROM EMPLOYEE
WHERE SALARY >= 3000000 AND SALARY < 5000000; -- 16행
	
-- EMPLOYEE 테이블에서
-- 급여가 300만 미만 또는 500만 이상인 
-- 사원의 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY 
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 이름이 '하'가 포함된 사원의
-- 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
...
/* ***** ORDER BY 절 *****
 * 
 * - SELET 조회 결과 집합(RESULT SET)을 
 * 	 원하는 순서로 정렬할 때 사용하는 구문
 * 
 * [작성법]
 * 3) SELECT 			(특정 컬럼만 선택해서 볼거야)
 ...
 * 
 * *** ORDER BY절은 무조건 SELET 마지막에 수행된다! ***
 * */
...
-- WHERE "직급코드" IN ('J4', 'J5', 'J6') 안되는 사유
--> WHERE절은 SELECT절보다 먼저 해석되는데 별칭은 SELECT절이 해석되어야
--  등록되기 때문에 WHERE절에서는 별칭 사용 불가능!!
...
-- 부서코드 오름차순, 급여 내림차순으로 정렬해서 조회
 --> 같은 부서코드끼리 모아놓고 그 안에서 급여 내림차순 정렬
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
