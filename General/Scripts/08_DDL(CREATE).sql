...
/* 자료형
    ...
    ** VARCAHR 2 주의사항 **
    - 오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
     	UTF-8은 영어/숫자 1 BYTE, 한글 3 BYTE로 저장하기 때문에
     	한글 데이터 저장 시 크기 계산을 주의해야 한다!!!
     	
    NVARCHAR2(문자 개수) : "가변" 길이 문자함(4000BYTE)    
...
--  컬럼 레벨 : 테니블 생성 시 생성할 컬럼의 정보를 작성하는 부분
...
--  ORA-00001: 무결성 제약 조건(KH_HYJ.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_HYJ.USER_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_HYJ.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
--ORA-00001: 무결성 제약 조건(KH_HYJ.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--ORA-01400: NULL을 ("KH_HYJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_HYJ.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_HYJ"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-02291: 무결성 제약조건(KH_HYJ.GRADE_CODE_FK1)이 위배되었습니다 
--	- 부모 키가 없습니다
...
FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KH_HYJ.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_HYJ.GENDER_CHECK)이 위배되었습니다
...
--         USER_ID(회원아이디) - 중복금지(ID_UK_USER_ID),
...
CREATE TABLE USER_TEST(
	...
	,CONSTRAINT ID_UK_USER_ID2 UNIQUE(USER_ID),
	CONSTRAINT UK_PNO2 UNIQUE (PNO),
	CONSTRAINT CK_GENDER2	CHECK (GENDER IN ( '남', '여') ),
	CONSTRAINT CK_STATUS2	CHECK (STATUS IN ('Y', 'N'))
);
