...
SELECT EMP_NAME, DEPT_CODE -- 메인 쿼리
FROM EMPLOYEE
WHERE DEPT_CODE IN(
...
-- 1) 급여를 300, 700만 받는 직원 (200만, 600만이 평균급여라 생각 할 경우)
...
WHERE EXISTS( -- 서브쿼리 조회 결과가 있으면 TRUE == 해당 행을 결과에 포함
	SELECT '있음(상관 ㄴ 구간)' -- 조회되는 데이터 여부가 중요!!
	FROM EMPLOYEE SUB
...
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE = (
	-- 메인쿼리 1행을 해석했을 때 조회되는 행 중에서 DEPT_CODE 값을 얻어와
	-- 서브쿼리에서 해당 DEPT_CODE가 일치하는 사원들 중 가장 빠른 입사일을 조회
